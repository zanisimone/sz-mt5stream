...
r"""Optionally set MT5_TERMINAL_PATH environment variable to the terminal path.
Example: C:\Program Files\MetaTrader 5\terminal64.exe"""

LOGIN = os.environ.get("MT5_LOGIN", None)
"""Optionally set MT5_LOGIN environment variable to your account login ID (integer),
otherwise reuse current session."""

PASSWORD = os.environ.get("MT5_PASSWORD", None)
"""Optionally set MT5_PASSWORD environment variable to your account password,
otherwise reuse current session."""

SERVER = os.environ.get("MT5_SERVER", None)
"""Optionally set MT5_SERVER environment variable to your account server name,
otherwise reuse current session."""

# Stream configuration
POLL_INTERVAL = 0.5
BARS_TIMEFRAME: Timeframe = "M1"  # Try M1, M5, M15, H1, or D1
ROLLING_TICKS = 1000
ROLLING_BARS = 100
...
# Show broker time
broker_dt = datetime.fromtimestamp(
    mt5.symbol_info_tick(SYMBOL).time, tz=timezone.utc
).replace(tzinfo=None)
...
try:
    ...
    while time.time() < end_time:
        ...
        if tick:
            broker_time = datetime.fromtimestamp(tick.time, tz=timezone.utc).replace(
                tzinfo=None
            )
        ...
        if not bars.empty:
            ...
            if last_candle_time is None or current_latest_time > last_candle_time:
                ...
                if num_new > 0:
                    print("\n" + "=" * 80)
                    print(
                        f">>> NEW CANDLE DETECTED: {BARS_TIMEFRAME} candle completed <<<".center(
                            80
                        )
                    )
                    print("=" * 80)
                    ...
                    if len(bars) >= 2:
                        ...
                        print(
                            f"  {'Time':<20} {'Open':>12} {'High':>12} {'Low':>12} {'Close':>12} {'Volume':>10}"
                        )
                        print(f"  {'-'*78}")
